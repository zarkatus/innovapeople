 ...
 (fol.data||[]).forEach(r=>{_s.folhaCount[r.cliente]=(_s.folhaCount[r.cliente]||0)+1;});
  ...
  function render(){
    ...
    html+='<div class="fl-kpis">'
      ...
      +'<div class="fl-kpi"><div class="lab">A processar</div><div class="val" style="'+(pend?'color:var(--warn, var(--ip-warn))':'')+'">'+pend+'</div><div class="note">pendentes de tratamento</div></div>'
  ...
  function cardFolha(r){
    ...
    return '<div class="fl-card'+(proc?' proc':'')+'" onclick="IpFolha._abrir(\''+esc(r.id)+'\')">'
      ...
      +'</div>'
      +'<div class="fl-card-side">'+estadoTag(r.estado)+(proc?'<span class="fl-mini">processada</span>':'<span class="fl-mini" style="color:var(--warn, var(--ip-warn))">a processar</span>')+'</div>'
    +'</div>';
  }
  ...
  function _abrir(id){
    ...
    if(colabs.length){
      ...
      body='<div class="fl-empty" style="padding:18px">Dados detalhados n&atilde;o estruturados como lista. Conte&uacute;do extra&iacute;do:<pre style="text-align:left;white-space:pre-wrap;font-family:var(--mono);font-size:11px;color:var(--ink2, var(--ip-text-body));margin-top:10px;max-height:260px;overflow:auto">'+esc(JSON.stringify(dados,null,2).slice(0,4000))+'</pre></div>';
    }
    const proc=!!r.processado_em;
