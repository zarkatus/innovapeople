...
(function(){
  ...
  async function open(container){
    ...
    _s.mandatos=_filtroEscopo((data||[]).filter(m=>m.status==='ativo'||!m.status)); _s.mandato=_s.mandatos[0]?.id||null;
    await reload(); go('mapa');
    // reescopo organizacional: ao trocar o nó na árvore, restringe a lista de empresas à subárvore (1×)
    if(window.IpOrg&&IpOrg.scope&&IpOrg.scope.onChange&&!_s._scoped){_s._scoped=true;
      IpOrg.scope.onChange(async function(){
        _s.mandatos=_filtroEscopo((data||[]).filter(m=>m.status==='ativo'||!m.status));
        if(!_s.mandatos.find(m=>m.id===_s.mandato))_s.mandato=_s.mandatos[0]?.id||null;
        await reload(); go(_s.tab||'mapa');
      });
    }
  }
  ...
  function mandatoSel(){
    if(!_s.mandatos.length)return `<div class="sk-warn">Nenhum trabalho ativo. Crie um trabalho no <a href="/sistema/" style="color:var(--acento)">Dashboard</a> antes de mapear competências.</div>`;
...
: await IpPersist.write(()=>sb().from('ip_skill_map').insert(row).select(),{label:'Avaliação 6D',offline:{table:'ip_skill_map',op:'insert',payload:row}});
  ...
  #sk-root .sk-tab.on{color:var(--acento);border-bottom-color:var(--acento)}
  ...
  #sk-root .sk-h{font-family:var(--mono);font-size:9.5px;letter-spacing:.2em;text-transform:uppercase;color:var(--acento-deep);font-weight:700;margin-bottom:12px}
  ...
  #sk-root .sk-field input:focus{outline:none;border-color:var(--acento)}
  #sk-root .sk-btn{padding:11px 20px;border:none;border-radius:8px;background:linear-gradient(135deg,var(--acento-glow, var(--ip-accent-light)),var(--acento));color:#2A1A3B;font-family:var(--sans);font-weight:600;font-size:13px;cursor:pointer;transition:all .2s}
  #sk-root .sk-btn:hover{transform:translateY(-1px);box-shadow:0 8px 22px rgba(200,89,36,.3)}
  ...
  #sk-root .sk-scale button.on{background:linear-gradient(135deg,var(--acento-glow, var(--ip-accent-light)),var(--acento));color:#2A1A3B;border-color:var(--acento)}
  ...
  #sk-root .sk-warn{padding:14px 18px;background:rgba(224,118,58,.1);border-left:2px solid var(--warn);border-radius:6px;color:var(--cream-muted);font-size:13px;font-style:italic;font-family:var(--serif);margin-bottom:14px}
  ...
  #sk-root .heat-cell.l2{background:rgba(224,118,58,.16);color:var(--warn);border-color:rgba(224,118,58,.28)}
  #sk-root .heat-cell.l3{background:rgba(123,165,216,.16);color:var(--info);border-color:rgba(123,165,216,.28)}
  #sk-root .heat-cell.l4{background:rgba(200,89,36,.16);color:var(--acento);border-color:var(--navy-edge)}
  #sk-root .heat-cell.l5{background:linear-gradient(135deg,var(--acento-glow, var(--ip-accent-light)),rgba(200,89,36,.4));color:#2A1A3B;border-color:var(--acento)}`;
